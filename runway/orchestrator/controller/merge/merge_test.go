...
func captureRegistry(t *testing.T, ctrl *gomock.Controller, publishErr error, captured *entityqueue.Message) consumer.TopicRegistry {
	...
	registry, err := consumer.NewTopicRegistry([]consumer.TopicConfig{
		{Key: topickey.TopicKeyMergeSignal, Name: "merger-signal", Queue: mockQ},
	})
...
func TestNewController(t *testing.T) {
	...
	assert.Equal(t, topickey.TopicKeyMerge, controller.TopicKey())
...
func TestProcess_Success(t *testing.T) {
	...
	expectedResult := entity.MergeResult{
		ID:      "queue-a/42",
		Success: true,
		Steps:   []entity.StepResult{{StepID: "queue-a/1", OutputIDs: []string{"abc123"}}},
	}
	mockVCS.EXPECT().Land(gomock.Any(), gomock.Any()).Return(expectedResult, nil)
	...
	assert.Equal(t, []string{"abc123"}, result.Steps[0].OutputIDs)
}

func TestProcess_Conflict(t *testing.T) {
	ctrl := gomock.NewController(t)
	mockVCS := vcsmock.NewMockVCS(ctrl)
	mockVCS.EXPECT().Land(gomock.Any(), gomock.Any()).Return(entity.MergeResult{}, fmt.Errorf("conflict in foo.go: %w", vcs.ErrConflict))
	...
	assert.Contains(t, result.Reason, "conflict")
}

func TestProcess_InfraError(t *testing.T) {
	ctrl := gomock.NewController(t)
	mockVCS := vcsmock.NewMockVCS(ctrl)
	mockVCS.EXPECT().Land(gomock.Any(), gomock.Any()).Return(entity.MergeResult{}, fmt.Errorf("vcs unavailable"))
...
func TestProcess_DeserializeError(t *testing.T) {
	...
	mockVCS := vcsmock.NewMockVCS(ctrl)
	controller := newTestController(t, ctrl, mockVCS, nil, nil)
	delivery := makeDelivery(t, ctrl, []byte(`not json`))
...
func TestProcess_PublishError(t *testing.T) {
	ctrl := gomock.NewController(t)
	mockVCS := vcsmock.NewMockVCS(ctrl)
	mockVCS.EXPECT().Land(gomock.Any(), gomock.Any()).Return(entity.MergeResult{ID: "queue-a/42", Success: true}, nil)
