...
//
// Error classification: deserialize, factory, and merge failures are
...
func (c *Controller) Process(ctx context.Context, delivery consumer.Delivery) (retErr error) {
	...
	if err := c.publish(ctx, runwaymq.TopicKeyMergeSignal, result, msg.PartitionKey); err != nil {
		...
		return errs.NewRetryableError(fmt.Errorf("failed to publish merge result for %s: %w", request.GetId(), err))
	}
