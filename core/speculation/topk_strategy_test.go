...
func TestTopKStrategy_Generate_NoDependencies(t *testing.T) {
	strategy := NewTopKStrategy(nil, DefaultK, 0)
...
func TestTopKStrategy_Generate_WithProbabilityFunc(t *testing.T) {
	...
	strategy := NewTopKStrategy(probFn, DefaultK, 0)
...
func TestTopKStrategy_Generate_CustomK(t *testing.T) {
	...
	strategy := NewTopKStrategy(probFn, 2, 0)
...
func TestTopKStrategy_Generate_MinScore(t *testing.T) {
	probFn := func(_ context.Context, ids []string) (map[string]float64, error) {
		probs := map[string]float64{"batch/0": 0.9, "batch/1": 0.8}
		return probs, nil
	}

	// With minScore=0.5, only the optimal path (0.9*0.8=0.72) should be returned.
	strategy := NewTopKStrategy(probFn, DefaultK, 0.5)
	...
	assert.Equal(t, "batch/2", tree.BatchID)
	assert.Len(t, tree.Speculations, 1)
	assert.InDelta(t, 0.72, float64(tree.Speculations[0].Score), 0.01)
}
