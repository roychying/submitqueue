...
**6. Ack** — Set `acked = TRUE`, advance contiguous watermark, update `offset_acked`, clean up delivery state behind watermark

**7. Nack** — Set `invisible_until = now + delay` for retry after backoff

**8. DLQ** — If `retry_count >= MaxAttempts`: atomically move message to DLQ topic (INSERT with DLQ topic + DELETE from original topic in transaction)
...
5. Cleans up delivery state rows behind the new watermark (non-fatal on failure)

The two-query approach avoids cross-table JOINs. Each store queries only its own table; the subscriber orchestrates both.

Watermark advancement is best-effort after a successful ack. If any step fails (get offsets, advance watermark, update offset), the failure is logged as a warning with a metric counter, but the ack itself has already succeeded. The system self-heals on the next ack cycle.
...
**Delivery State Cleanup:** The `AdvanceWatermark` cleanup DELETE only removes delivery state rows (not messages) behind the contiguous watermark. Failure is non-fatal — logged as a warning, cleaned up on the next cycle.
...
**What happens:**
1. Message is marked acked in delivery state (`invisible_until = MAX_BIGINT`)
...
All metrics use `metrics.NamedCounter`, `metrics.NamedGauge`, and `metrics.NamedTimer` from `core/metrics`. Store-level operations use `metrics.Begin`/`Complete` for latency and success/failure tracking.

**Publisher:**
- `messages_published`, `publish_errors`

**Subscriber — Delivery Operations:**
- `messages_acked`, `messages_nacked`, `messages_rejected_to_dlq`, `messages_rejected_no_dlq`
- `visibility_extended`

**Subscriber — Ack Cleanup (non-fatal, Warnw on failure):**
- `ack.get_offset_errors`, `ack.get_offsets_errors`, `ack.advance_watermark_errors`, `ack.update_offset_errors`

**Subscriber — Reject Cleanup (non-fatal, Warnw on failure):**
- `reject.mark_acked_errors`, `reject.update_offset_errors`

**Subscriber — Poll:**
- `poll.messages_received`, `poll.message_age`, `poll.latency`
- `poll.gc_errors`, `poll.mark_acked_after_retry_limit_errors`, `poll.update_offset_after_retry_limit_errors`

**Subscriber — DLQ:**
- `dlq_move.messages_moved_to_dlq`, `dlq_move.errors`

**Subscriber — Leasing:**
- `discover.leases_acquired`
- `lease_renewal.renew_errors`, `lease_renewal.get_partitions_errors`
- `lease_release.errors`

**Subscriber — Heartbeats:**
- `heartbeat.sent`, `heartbeat.errors`
- `heartbeat.deregistrations`, `heartbeat.deregister_errors`

**Subscriber — Fair Share and Rebalance:**
- `subscribe.active_subscriptions`
- `fair_share.active_subscribers`, `fair_share_cap.errors`
- `rebalance.partitions_released`, `rebalance.release_errors`
- `discover_partitions.errors`

**Stores (via `metrics.Begin`/`Complete`):**
- Each operation has `.succeeded`, `.failed`, `.latency` (e.g., `message_store.insert.succeeded`, `delivery_state_store.mark_acked.latency`)
...
|-------|-----------|---------|
| Debug | Normal operational details | Message fetch counts, partition worker start/stop, subscription creation, lease release, GC deleted count |
| Info | Significant state changes | Publish success, DLQ moves, partition acquisition, rebalance releases, subscriber open/close |
| Warn | Non-fatal failures, system self-heals | Watermark advance failure, offset update failure, GC failure, lease renewal failure, heartbeat failure, worker stop timeout |
| Error | Failures that abort the current operation | Offset init failure, message fetch failure, deliverability check failure, mark delivered failure |

**Structured fields:** `topic`, `partition_key`, `message_id`, `offset`, `retry_count`, `error`, `watermark`, `consumer_group`

### Error Handling Architecture

Stores return errors via `fmt.Errorf` — they do not log errors themselves (no double-logging). The subscriber layer decides the severity:

- **Errors that abort the operation** (e.g., failed to fetch messages) → `Errorw` + return/continue
- **Non-fatal cleanup failures** where the primary operation already succeeded (e.g., failed to advance watermark after ack) → `Warnw` + metric counter, system self-heals on next cycle
- **Debug-level** for normal operational details (fetch counts, GC results)

All `result.RowsAffected()` errors are checked and logged as Warnw (no swallowed errors).
