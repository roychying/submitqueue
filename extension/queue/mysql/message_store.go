...
// It finds the minimum offset_acked across all consumer groups for this partition,
// then deletes messages with offset <= that minimum.
// Returns the number of rows deleted.
func (s *sqlmessageStore) GarbageCollect(ctx context.Context, topic string, partitionKey string) (_ int64, retErr error) {
	op := metrics.Begin(s.scope, "gc", metrics.NewTag("topic", topic))
	defer func() { op.Complete(retErr) }()

	// Find the minimum acked offset across all consumer groups
	...
	if err != nil {
		return 0, fmt.Errorf("query min offset topic=%s partition=%s: %w", topic, partitionKey, err)
	}
	...
	`, MessagesTableName), topic, partitionKey, minOffset.Int64)
...
func (s *sqlmessageStore) GetOffsetsAbove(ctx context.Context, topic string, partitionKey string, afterOffset int64) (_ []int64, retErr error) {
	...
	rows, err := s.db.QueryContext(ctx, fmt.Sprintf(`
		...
		WHERE topic = ? AND partition_key = ? AND offset > ?
		ORDER BY offset ASC
	`, MessagesTableName), topic, partitionKey, afterOffset)
	if err != nil {
