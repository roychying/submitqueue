...
func (s *sqlpartitionLeaseStore) DiscoverAndAcquirePartitions(ctx context.Context, topic string, subscriberName string, consumerGroup string, leaseDurationMs int64, maxPartitions int) (_ int, retErr error) {
	...
	if err != nil {
		return 0, fmt.Errorf("discover partitions topic=%s: %w", topic, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&partitionKey); err != nil {
			return 0, fmt.Errorf("scan partition key topic=%s: %w", topic, err)
		}
		partitions = append(partitions, partitionKey)
	}

	if err := rows.Err(); err != nil {
		return 0, fmt.Errorf("row iteration topic=%s: %w", topic, err)
	}
	...
	if maxPartitions > 0 {
		...
		if err != nil {
			return 0, fmt.Errorf("get owned partitions for cap check topic=%s: %w", topic, err)
		}
...
func (s *sqlpartitionLeaseStore) DiscoverPartitions(ctx context.Context, topic string) (_ []string, retErr error) {
	op := metrics.Begin(s.scope, "discover_partitions", metrics.NewTag("topic", topic))
	defer func() { op.Complete(retErr) }()
