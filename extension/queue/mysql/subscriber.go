...
func (d *sqlDelivery) Ack(ctx context.Context) error {
	...
	// Advance the watermark and update offset_acked.
...
func (d *sqlDelivery) Reject(ctx context.Context, reason string) error {
	...
	if d.dlqConfig.Enabled {
		...
		if err := d.subscriber.offsetStore.UpdateAckedOffset(
			...
			return fmt.Errorf("update offset after DLQ move: %w", err)
		}
...
func (s *subscriber) managePartitions(ctx context.Context, sub *subscription) {
	...
	for {
		...
		case <-leaseTicker.C:
			...
			// Stores already emit per-operation metrics via Begin/Complete.
			if err := s.rebalance(ctx, sub); err != nil {
				s.logger.Errorw("rebalance failed", append(logFields, "error", err)...)
			}
			if err := s.renewLeases(ctx, sub); err != nil {
				s.logger.Errorw("lease renewal failed", append(logFields, "error", err)...)
			}
			if err := s.sendHeartbeat(ctx, sub); err != nil {
				s.logger.Errorw("heartbeat failed", append(logFields, "error", err)...)
			}

		case <-discoveryTicker.C:
			if err := s.discoverAndReconcileWorkers(ctx, sub); err != nil {
				s.logger.Errorw("partition discovery failed", append(logFields, "error", err)...)
			}
		}
	}
}
...
func (s *subscriber) discoverAndReconcileWorkers(ctx context.Context, sub *subscription) error {
	...
	maxPartitions, _, err := s.fairShareCap(ctx, sub)
	if err != nil {
		return fmt.Errorf("compute fair share cap: %w", err)
	}

	// Discover and try to acquire leases for new partitions
	if _, err := s.leaseStore.DiscoverAndAcquirePartitions(ctx, sub.topic, cfg.SubscriberName, cfg.ConsumerGroup, cfg.LeaseDurationMs, maxPartitions); err != nil {
...
func (w *partitionWorker) pollAndDeliver(ctx context.Context) error {
	...
	for _, row := range rows {
		...
		if err != nil {
			return fmt.Errorf("check deliverability offset=%d: %w", row.Offset, err)
		}
		if !deliverable {
			continue
		}
		...
		if err != nil {
			return fmt.Errorf("get retry count offset=%d: %w", row.Offset, err)
		}
		...
		if retryCount >= cfg.Retry.MaxAttempts {
			...
			if err := s.offsetStore.UpdateAckedOffset(ctx, sub.topic, partitionKey, row.Offset, cfg.ConsumerGroup); err != nil {
				return fmt.Errorf("update offset after retry limit message=%s: %w", row.ID, err)
			}
			continue
		}
...
func (s *subscriber) renewLeases(ctx context.Context, sub *subscription) error {
	cfg := sub.config
	leasedPartitions, err := s.leaseStore.GetLeasedPartitions(ctx, sub.topic, cfg.SubscriberName, cfg.ConsumerGroup)
	if err != nil {
		return fmt.Errorf("get leased partitions for renewal: %w", err)
	}
...
func (s *subscriber) rebalance(ctx context.Context, sub *subscription) error {
	cfg := sub.config
...
func (s *subscriber) fairShareCap(ctx context.Context, sub *subscription) (int, []string, error) {
	...
	if err != nil {
		// Graceful degradation: use owned partitions only for fair share computation.
		// Discovery failure should not block rebalancing — owned partitions are
		// sufficient for a conservative estimate. Retried on the next tick.
		s.logger.Warnw("discover partitions failed, using owned partitions only",
			"topic", sub.topic,
			"consumer_group", cfg.ConsumerGroup,
			"error", err,
		)
