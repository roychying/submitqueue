...
Each partition worker runs independently — polls the DB on a ticker, checks deliverability per message, and sends deliveries to the shared channel. A slow or blocked partition does not affect other partitions.
...
Stores do not log errors — they return them. The subscriber layer classifies severity:
- **Errorw** — failures that abort the current operation (fetch, deliverability check)
- **Warnw** — non-fatal cleanup failures where the system self-heals (watermark, offset update, GC)
- **Debugw** — normal operational details (fetch counts, worker lifecycle)
